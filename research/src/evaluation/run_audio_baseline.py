"""
Audio baseline evaluation script.

Runs audio engines against SortingTask and AudioRetrievalTask on ESC-50,
then appends dated entries to results/log.json.

Usage:
    python -m src.evaluation.run_audio_baseline
"""
...
from src.data_utils.audio_loader import load_esc50
from src.engines.audio.mfcc import MFCCEngine
from src.engines.audio.clap import CLAPEngine
from src.tasks.sorting import SortingTask
from src.tasks.audio_retrieval import AudioRetrievalTask

# Config
N_RETRIEVAL_QUERIES = 50
RETRIEVAL_K = 5
# Caps clips per class, set to None to use the full dataset
# (ESC-50 has 32 train clips / 8 test clips per class)
...
def main():
    ...
    train_paths, train_labels = load_esc50(subset="train", max_per_class=MAX_PER_CLASS_TRAIN)
    test_paths, test_labels = load_esc50(subset="test", max_per_class=MAX_PER_CLASS_TEST)
    n_clusters = len(set(train_labels))

    engines = {
        "MFCC": MFCCEngine(n_mfcc=20),
        "CLAP-HTSAT": CLAPEngine("laion/clap-htsat-unfused"),
    }
    ...
    for name, engine in engines.items():
        ...
        sorting_metrics = SortingTask(n_clusters=n_clusters).run(engine, (train_paths, train_labels))
        ...
        print(f"Running AudioRetrievalTask (k={RETRIEVAL_K})...")
        retrieval_metrics = AudioRetrievalTask(k=RETRIEVAL_K).run(engine, (train_paths, queries))
