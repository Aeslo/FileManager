"""
Audio-to-Audio Retrieval Task.

Identical to RetrievalTask but uses embed_audio() for query encoding
instead of embed_text(), since queries are audio file paths.
"""
...
class AudioRetrievalTask(RetrievalTask):
    """Retrieval task where both query and corpus are audio file paths.

    dataset: (corpus_paths, queries)
      corpus_paths : list[str]
      queries : list[tuple[str, set[int]]] - (query_audio_path, relevant_indices)
    """
    ...
    def run(self, engine, dataset: tuple[list[str], list[tuple[str, set[int]]]]) -> dict[str, float]:
        ...
        for query_path, relevant in queries:
            query_vec = engine.embed_audio(query_path)
