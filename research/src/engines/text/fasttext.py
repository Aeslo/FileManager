...
class FastTextEngine(BaseEngine):
    ...
    Parameters
    ---
    vector_size : Dimensionality of subword vectors (default 100)
    window : Max distance between center and context word (default 5)
    min_count : Minimum word frequency to include in vocabulary (default 2)
    negative : Number of negative samples per positive pair (default 5)
    epochs : Number of passes over the corpus (default 5)
    lr : Initial learning rate, linearly decayed to lr/100 (default 0.025)
    min_n, max_n : Character n-gram size range (default 3-5)
    """
